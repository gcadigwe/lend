...
import { CurrencyAmount, ETHER, BNB, MATIC, AVAX, SwapParameters, Token, Trade, TradeOptionsDeadline, TradeType } from '@spherium/swap-sdk'
...
export default function v1SwapArguments(
  ...
  const inputETH = (trade.inputAmount.currency === ETHER || trade.inputAmount.currency === BNB || trade.inputAmount.currency === MATIC || trade.inputAmount.currency === AVAX)
  const outputETH = (trade.outputAmount.currency === ETHER || trade.outputAmount.currency === BNB || trade.outputAmount.currency === MATIC || trade.outputAmount.currency === AVAX)
