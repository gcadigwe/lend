...
function Balance({ balance }: { balance: any }) {
  return <StyledBalanceText title={parseFloat(balance).toString()}>{parseFloat(balance)}</StyledBalanceText>
}
...
function CurrencyRow({
  ...
  const getRightCurrency = (curr:any) => {
    ...
    if(chainId ===1) {
      if(curr?.name === "SPHRI") {return balanceinEther && balanceinEther[0]?.balanceInEther}
      if(curr?.name === 'BACKED') {return balanceinEther && balanceinEther[1]?.balanceInEther}
    }
    else if(chainId ===56) {
      if(curr?.name === "Spherium") {return balanceinEther &&  balanceinEther[0]?.balanceInEther}
      if(curr?.name === 'BACKED') {return  balanceinEther && balanceinEther[1]?.balanceInEther}
    }
  }
  ...
  return (
    ...
    >
      ...
      <Column>
        ...
        <TYPE.darkGray ml="0px" fontSize={'12px'} fontWeight={300} color={"#fff"}>
          {currency.name} {!isOnSelectedList && customAdded && '• Added by user'}
        </TYPE.darkGray >
      </Column>
      <TokenTags currency={currency} />
      <RowFixed style={{ justifySelf: 'flex-end' }} fontWeight={500} fontSize={16} color={'#fff'}>
      {chainId!==1 && chainId!==56 ? (currentBalance ? <Balance balance={currentBalance} /> : account ? <Loader /> : null) : (getRightCurrency(currency) ? <Balance balance={getRightCurrency(currency)} /> : account ? <Loader /> : null)}
      </RowFixed>
    </MenuItem>
  )
}
            ...
            <LightGreyCard padding="8px 12px" borderRadius="8px">
              <RowBetween>
                <RowFixed>
                  ...
                  <TYPE.main ml="6px" fontSize="12px" color={'white'}>
